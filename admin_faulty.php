         ...
         <a href="#"><!-- a href begin -->
          <form method="post">
                <div class="panel-footer">
                  ...
                  <input type="submit" class="btn btn-info" id="search" name="Search1" value="Search">
                  <!-- panel-footer begin -->
               </div>
             </form>
             </a>
...
<?php 
    ...
    if (isset($_POST["Search1"])){
      ...
      include 'open_db.php';
      $selecttxt1 = $_POST["Fromdate1"];
      $queryres = mysqli_query($con,"SELECT * FROM tbl_users INNER JOIN tbl_records ON tbl_records.users_id=tbl_users.id WHERE dateToday = '$selecttxt1' AND time_in = 'ONTIME'");

       $queryres = "SELECT * FROM tbl_users INNER JOIN tbl_records ON tbl_records.users_id=tbl_users.id WHERE CONCAT(dateToday) LIKE '%".$selecttxt1."%' AND time_in = 'ONTIME'";
      $search_result = filterTable($queryres);
    }
    else{

       $currentDate = date("d-m-Y");
       $queryres = "SELECT * FROM tbl_users INNER JOIN tbl_records ON tbl_records.users_id=tbl_users.id, WHERE dateToday = '$currentDate' AND time_in = 'ONTIME' AND time_in = 'LATE' AND time_stamp BETWEEN start_time AND end_time";
       $search_result = filterTable($queryres);
               ...
               <li class="nav-item">
                 <a class="nav-link active" href="admin_dash.php">On-Time</a>
               </li>
               <li class="nav-item">
                 <a class="nav-link" href="admin_late.php">Late</a>
               </li>
               <li class="nav-item">
